...
async function main() {
  ...
  const client = new Client({ name: 'benchmark-gemini', version: '2.0.0' }, { capabilities: {} });
  ...
  const outPath = path.join(__dirname, '..', 'dist', 'token_audit_gemini.json');
  ...
  await fs.writeFile(path.join(scratchDir, 'token_audit_gemini.js'), shim);
  await fs.writeFile(path.join(__dirname, '..', 'dist', 'token_audit_gemini.js'), shim);
